/**
 * MINDSET - NOTIFICATIONS ENGINE v13.0
 * ARQUIVO COMPLETO: Reset de Cache + Sincronia Firebase + Vozes Revisadas
 */
...
/**
 * SINCRONIZAÇÃO COM FIREBASE E ONESIGNAL
 */
window.sincronizarMindsetOneSignal = async (user) => {
    ...
    window.OneSignalDeferred.push(async function(OneSignal) {
        ...
        } catch (e) { console.warn("Erro OneSignal:", e); }
    });

    try {
        if (window.db) {
            const { doc, updateDoc } = await import("https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js");
            await updateDoc(doc(window.db, "users", targetId), {
                setup: user.setup,
                nome: user.nome,
                pendentes: pendentes,
...
/**
 * ATIVAR NOTIFICAÇÕES (O SCRIPT DE RESET COMPLETO)
 */
window.ativarNotificacoesManual = async () => {
    if (window.showModal) {
        window.showModal("SISTEMA", "Iniciando configuração de alertas... Por favor, autorize a permissão no navegador quando solicitado.");
    }

    (async () => {
        console.log("🧨 EXECUTANDO RESET DE CACHE...");
            ...
            try {
                ...
                const checkInterval = setInterval(async () => {
                    ...
                    if (permission === "granted") {
                        ...
                        if (raw) await window.sincronizarMindsetOneSignal(JSON.parse(raw));
                        if (window.showModal) window.showModal("SUCESSO", "Notificações ativadas!");
                    } else if (permission === "denied") {
                        clearInterval(checkInterval);
                    }
                }, 1000);
            } catch (e) { console.error("Erro no fluxo OneSignal:", e); }
        });
    })();
};
